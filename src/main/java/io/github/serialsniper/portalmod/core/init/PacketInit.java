...
//                .encoder(RadioStatePacket.Server::encode).decoder(RadioStatePacket.Server::new)
//                .consumer(RadioStatePacket.Server::handle).add();

        INSTANCE.messageBuilder(RadioUpdateClientPacket.class, ID++, NetworkDirection.PLAY_TO_CLIENT)
                .encoder(RadioUpdateClientPacket::encode).decoder(RadioUpdateClientPacket::new)
                .consumer(RadioUpdateClientPacket::handle).add();

//        INSTANCE.messageBuilder(AntlineUpdatePacket.Server.class, ID++, NetworkDirection.PLAY_TO_SERVER)
//                .encoder(AntlineUpdatePacket.Server::encode).decoder(AntlineUpdatePacket.Server::new)
//                .consumer(AntlineUpdatePacket.Server::handle).add();
//
        INSTANCE.messageBuilder(AntlineUpdatePacket.Client.class, ID++, NetworkDirection.PLAY_TO_CLIENT)
                .encoder(AntlineUpdatePacket.Client::encode).decoder(AntlineUpdatePacket.Client::new)
                .consumer(AntlineUpdatePacket.Client::handle).add();
    }
}